...
<%@ include file="../includes/header.jsp" %>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
				...
				<!-- 첨부파일 이미지 영역 끝 -->
				<div class="getbtns">
					<button onclick="location.href='${ctx}/board/review?pageNum=${cri.pageNum}&amount=${cri.amount }&type=${cri.type}&keyword=${cri.keyword}'">목록</button>
					<!-- 변수등록 -->
					<sec:authentication property="principal" var="pinfo"/>
					<sec:authorize access="isAuthenticated()">
					<!-- 로그인한 사용자랑 글쓴이가 같아야 수정, 삭제보임 -->
					<c:if test="${pinfo.username eq board.writer}">
					<button class="modify-btn" onclick="location.href='${ctx}/board/modify?bno=${board.bno}&pageNum=${cri.pageNum}&amount=${cri.amount }&type=${cri.type}&keyword=${cri.keyword}'">수정</button>
					</c:if>
					...
					<form method="post" action="${ctx}/board/remove">
		...
		$(document).ready(function(){
			...
			(function(){
				...
				$.getJSON("${ctx}/board/getAttachList",{bno:bno},
					function(arr){
						console.log(arr);
						//이미지 나타내기
						let str = "";
						...
						$(arr).each(function(i, attach){
							...
							str += "<div><img src='${ctx}/display?fileName="+fileCallpath+"'/>";
							str += "</div></li>";
							view += "<img src='${ctx}/display?fileName="+fileCallpath2+"'/>";
						})
			...
			function showImage(fileCallPath){
				...
				$(".bigPicture").html("<img src='${ctx}/display?fileName="+fileCallPath+"' width='500'/><span>X</span>");
			}
